...
ImageFactory::~ImageFactory() {
  ...
  std::map<std::string , std::vector<SDL_Surface*> >::iterator mptr = multiSurfaces.begin();
  while(mptr != multiSurfaces.end())
  {
    for (unsigned int i = 0; i < mptr->second.size(); ++i) {
      SDL_FreeSurface( mptr->second[i] );
    }
    ++mptr;
  }
